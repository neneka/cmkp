      ...
      v-card
        ...
        v-card-text
          v-form(v-model="addItemDialog.valid")
            ...
            v-select(hide-no-data hide-selected :item-text="v => `${v.locationString} ${v.name} ${v.author}`" item-value="id" label="サークル選択" return-object placeholder="サークル名または作家名を選択" :items="searchCircles" :loading="$apollo.queries.searchCircles.loading" required :rules="[rules.required]")
