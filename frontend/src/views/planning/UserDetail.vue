            ...
            v-container(fluid, grid-list-xs)
              v-layout(row, wrap)
                ...
                v-flex(v-for="i in 2", xs12, sm4, md4, lg4, :key="i")
                  circle-priority-list(
                    :title="`${i}日目`",
                    :circles="priorities[i]"
                  )
        v-flex(d-flex, xs12)
          v-card
            v-card-title
            ...
            v-container(fluid, grid-list-xs)
              ...
              v-radio-group(v-model="filter.day", row)
                ...
                v-radio(
                  v-for="i in 2",
                  :label="`${i}日目(${requestedCircleCounts[i]})`",
                  :value="i",
                  :key="i"
                )
              v-layout(row, wrap)
                v-flex(
                  ...
                  v-card
                    v-card-title.headline.lighten-4(
                      ...
                      ) {{ circle.locationString }} {{ circle.name }}
                    v-divider
                    v-list
                      v-list-tile(
                        v-for="item in circle.items",
                        :key="item.id",
                        @click="onItemClicked(circle, item)"
                      )
                        v-list-tile-content
    ...
    v-card
      ...
      v-card-text
        v-form(v-model="editItemDialog.valid")
          v-text-field(
            v-model.number="editItemDialog.price",
            label="単体価格",
            hint="決定していない場合は空欄にしてください",
            type="number",
            min="0",
            max="50000",
            persistent-hint,
            :disabled="editItemDialog.sending"
          )
          v-text-field(
            v-model.number="editItemDialog.num",
            label="個数",
            type="number",
            min="1",
            max="99",
      ...
      v-card-actions
        ...
        v-btn(
          flat,
          @click.native="editItemDialog.open = false",
          :disabled="editItemDialog.sending"
        ) キャンセル
        v-btn(
          flat,
          color="red",
          @click.native="deleteRequest",
          :disabled="editItemDialog.sending",
          :loading="editItemDialog.deleting"
        ) 削除
        v-btn(
          flat,
          color="primary",
          @click.native="editRequest",
          :disabled="editItemDialog.sending || !editItemDialog.valid",
          :loading="editItemDialog.editing"
        ) 修正
    ...
    v-card
      v-toolbar(card)
        ...
        v-toolbar-items
          v-btn(
            color="primary",
            :disabled="!addItemDialog.valid || addItemDialog.sending",
            :loading="addItemDialog.sending",
            @click="createRequest"
          ) 登録
      v-card-text
        v-form(v-model="addItemDialog.valid")
          v-autocomplete(
            ...
            :items="searchCircles",
            v-model="addItemDialog.circle",
            :loading="$apollo.queries.searchCircles.loading",
            :search-input.sync="addItemDialog.query",
            clearable,
            required,
            :rules="[rules.required]"
          )
          v-select(
            v-model="addItemDialog.selectedItem",
            :items="selectableCircleItems",
            label="商品名を選択",
            hint="希望の商品が無い場合は新規登録を選んでください",
            persistent-hint,
            return-object,
            single-line,
            item-text="name",
            item-value="id",
            :loading="$apollo.queries.circleItems.loading"
          )
          v-text-field(
            v-model="addItemDialog.name",
            label="商品名を入力",
            hint="曖昧な商品名を入力しないでください。また複数の商品を一つにまとめて登録しないでください。(OK：新刊A, NG：新刊AとB)",
            required,
            :rules="[rules.required]",
            maxLength="100",
            counter,
            persistent-hint,
            :disabled="addItemDialog.selectedItem == null || !isNewItem"
          )
          v-text-field(
            v-model.number="addItemDialog.price",
            label="単体価格",
            hint="決定していない場合は空欄にしてください",
            type="number",
            min="0",
            max="50000",
            persistent-hint,
            :disabled="addItemDialog.selectedItem == null"
          )
          v-text-field(
            v-model.number="addItemDialog.num",
            label="個数",
            type="number",
            min="1",
            max="99",
...
export default {
  ...
  computed: {
    ...
    priorities: function () {
      return [
        this.fetchData.priority0.circles,
    ...
    isNewItem: function () {
      return (
        ...
        this.addItemDialog.selectedItem.id === null
      );
    },
  },
  watch: {
    "addItemDialog.selectedItem": function () {
      ...
      if (this.isNewItem) {
        ...
        this.addItemDialog.price =
          this.addItemDialog.selectedItem.price >= 0
            ? this.addItemDialog.selectedItem.price
            : "";
        this.addItemDialog.num = 1;
      }
    },
  ...
  apollo: {
    ...
    searchCircles: {
      ...
      skip: function () {
        return (
          this.addItemDialog.query === "" || this.addItemDialog.circle != null
        );
      },
    },
    circleItems: {
  ...
  methods: {
    ...
    createRequest: async function () {
      ...
      try {
        ...
        if (!this.isNewItem) {
          ...
          if (
            this.addItemDialog.price !== this.addItemDialog.selectedItem.price
          ) {
    ...
    editRequest: async function () {
      ...
      try {
        ...
        const p =
          this.editItemDialog.price === "" ? -1 : this.editItemDialog.price;
