...
v-container(fluid, grid-list-md)
  ...
  v-progress-linear(v-if="$apollo.queries.users.loading", indeterminate)
  template(v-else)
    v-layout(row, wrap)
          ...
          v-card
            ...
            v-card-actions
              ...
              v-btn(
                depressed,
                small,
                @click.stop="openChangePermissionDialog(user)"
              ) 権限変更
  v-btn(
    fixed,
    dark,
  ...
  computed: {
    filteredUsers: function () {
      return this.users.filter((v) =>
        this.filterDay ? v.entries.includes(this.filterDay) : true
      );
    },
  },
  methods: {
